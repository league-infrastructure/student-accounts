...
- [x] `server/src/services/group.service.ts` exports `GroupService`.
- [x] Constructor: `new GroupService(prisma, audit: AuditService)`.
- [x] `create({ name, description? }, actorId)`:
...
- [x] `update(id, { name?, description? }, actorId)`:
...
- [x] `findById(id)` — throws `NotFoundError` if missing.
- [x] `findAll()` — returns `{ id, name, description, memberCount,
      createdAt }[]` ordered by name.
- [x] `listMembers(groupId)` — returns `{ group, users }` shape
      aligned with `/admin/cohorts/:id/members` (users with
      `externalAccounts` projected the same way).
- [x] `addMember(groupId, userId, actorId)`:
      ...
      - Inside one transaction: creates the `UserGroup` row and
        ...
        `details = { group_name }`.
- [x] `removeMember(groupId, userId, actorId)`:
...
- [x] `searchUsersNotInGroup(groupId, q, limit?)`:
...
- [x] `listGroupsForUser(userId)` returns `{ id, name }[]` for the
      groups this user belongs to, ordered by name.
- [x] Registered on `ServiceRegistry` as `groups: GroupService`.
- [x] Request type augmentation (`server/src/contracts`)
      includes `groups` on `req.services` if the typing file lists
      service properties explicitly.
- [x] Unit tests in `tests/server/services/group.service.test.ts`
