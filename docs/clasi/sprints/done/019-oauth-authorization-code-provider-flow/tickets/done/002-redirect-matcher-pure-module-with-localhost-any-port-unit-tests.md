...
- [x] `server/src/services/oauth/redirect-matcher.ts` exports `matchesRedirectUri(registered, candidate)`.
- [x] Function is pure (no imports of `prisma`, `fetch`, `fs`, etc.).
- [x] JSDoc documents the loopback-equivalence rule and the localhost-any-port rule.
- [x] Returns `false` (not throws) on malformed candidate URLs.
- [x] All 15+ unit tests pass (20 written).
