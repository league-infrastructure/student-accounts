...
- [x] File created at `server/src/services/passphrase.service.ts`.
- [x] Exports `PassphraseService` class with the polymorphic `Scope` type:
  ```ts
  type Scope = { kind: 'group' | 'cohort'; id: number }
  ```
- [x] `create(scope, opts: { plaintext?: string; grantLlmProxy: boolean }, actorId: number)`:
...
- [x] `findBySignupValue(plaintext: string)`:
...
- [x] `server/src/services/service.registry.ts` registers `passphrases: new PassphraseService(prisma)` (or however the registry wires services).
- [x] Existing services are unchanged; no circular imports introduced.

### Tests

- [x] `tests/server/services/passphrase.service.test.ts` created and green:
