...
status: todo
use-cases: ["SUC-012-001", "SUC-012-002", "SUC-012-003", "SUC-012-004", "SUC-012-005", "SUC-012-006", "SUC-012-007", "SUC-012-008"]
depends-on: ["002", "003"]
github-issue: ""
todo: ""
---
...
- [ ] `server/src/routes/admin/groups.ts` exports `adminGroupsRouter`.
- [ ] Router is mounted in `server/src/routes/admin/index.ts` via
      `adminRouter.use('/admin', adminGroupsRouter)` with
      `requireAuth` + `requireRole('admin')` inherited.
- [ ] Routes implemented (all under `/admin`):
...
- [ ] Error responses: 404 for missing group/user, 409 for duplicate
      name / duplicate membership, 422 for blank name, 400 for bad
      body / invalid `accountType`.
- [ ] `AppError` subclasses map to their `statusCode`. Anthropic
...
- [ ] Actor id is sourced from `(req.session as any).userId`.
- [ ] Integration tests in
      ...
      endpoint.
- [ ] `user-search` returns `[]` on a query shorter than 2 characters
