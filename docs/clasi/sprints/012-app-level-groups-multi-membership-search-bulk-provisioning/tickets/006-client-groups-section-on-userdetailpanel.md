...
- [ ] A new `AccountCard`-style section titled "Groups" is rendered
...
- [ ] The section fetches `GET /api/admin/users/:id/groups` on mount
      and whenever the user id changes.
- [ ] Each membership is shown with a Remove button. Clicking calls
      `DELETE /api/admin/groups/:groupId/members/:userId`, confirms
      first, and re-fetches on success.
- [ ] Below the list: a combobox of groups the user is not in
...
- [ ] Empty state: "Not in any groups yet." with the add combobox
      visible.
- [ ] A narrow, self-contained test in
