...
- [ ] `prisma/schema.prisma` has the three new models with the fields and indexes above.
- [ ] `OAuthClient` gains the back-relations (`authorization_codes`, `refresh_tokens`, `consents`) so cascade deletes compile.
- [ ] `User` gains the back-relations (`oauth_authorization_codes`, `oauth_refresh_tokens`, `oauth_consents`).
- [ ] `prisma db push --accept-data-loss` runs cleanly against dev DB.
- [ ] `prisma generate` runs cleanly; generated client exposes the new model types.
- [ ] `npm run test:server` still green (no new tests this ticket — schema-only).
