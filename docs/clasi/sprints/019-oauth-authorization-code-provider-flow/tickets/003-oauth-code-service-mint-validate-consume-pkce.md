...
- [ ] `server/src/services/oauth/oauth-code.service.ts` exists, registered as `oauthCodes` in `service.registry.ts`.
- [ ] `mint` returns plaintext once; only SHA-256 hash is persisted (`code_hash` column).
- [ ] `mint` rejects `code_challenge_method !== 'S256'` with `invalid_request`.
- [ ] `consume` is single-use enforced atomically (transactional `update` with `consumed_at: null` predicate).
- [ ] `consume` validates expiry, `redirect_uri` exact match, and PKCE S256 verifier.
- [ ] All failure paths throw `OAuthError` with OAuth-spec error codes.
- [ ] Audit events `oauth_code_issued` and `oauth_code_consumed` are written.
