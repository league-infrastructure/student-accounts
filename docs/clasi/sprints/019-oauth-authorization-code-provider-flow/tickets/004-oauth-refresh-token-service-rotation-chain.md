...
- [ ] `server/src/services/oauth/oauth-refresh.service.ts` exists, registered as `oauthRefreshTokens`.
- [ ] `mint` returns plaintext once; only SHA-256 hash persisted.
- [ ] `rotate` happy path returns `{ refresh_token, access_token, expires_in, scopes }`; old row's `replaced_by_id` points to new row.
- [ ] Replay of an already-rotated token revokes the entire chain (every row's `revoked_at` is set) and throws `invalid_grant`.
- [ ] Refresh from a disabled client throws `invalid_client`.
- [ ] Expired refresh → `invalid_grant`.
- [ ] Revoked refresh → `invalid_grant`.
- [ ] Audit events: `oauth_refresh_minted`, `oauth_refresh_rotated`, `oauth_refresh_reuse_detected` written at appropriate points.
