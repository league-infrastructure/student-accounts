...
- [ ] `POST /admin/groups/:id/passphrase` — identical semantics, fires `adminBus.notify('groups')`.
- [ ] `GET /admin/groups/:id/passphrase` — identical semantics.
- [ ] `DELETE /admin/groups/:id/passphrase` — identical semantics, fires `adminBus.notify('groups')`.

### Tests

- [ ] `tests/server/routes/admin-passphrase.test.ts` created and green, covering all six routes:
