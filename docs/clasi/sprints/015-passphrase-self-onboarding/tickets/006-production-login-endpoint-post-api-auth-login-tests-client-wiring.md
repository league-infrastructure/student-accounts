...
- [ ] File created; exports an Express `Router` with `POST /` mounted at `/api/auth/login` via `server/src/routes/auth.ts`.
- [ ] Route is public (no `requireAuth` middleware).
- [ ] Handler:
  1. Validates body: `username` (string, required) and `password` (string, required). Returns `400` for missing fields.
  ...
  5. Sets `req.session.userId = user.id`.
  6. Returns `200 { id, displayName, primaryEmail, role }`.
- [ ] `/api/auth/test-login` is not modified.

### Tests

- [ ] `tests/server/routes/auth-login.test.ts` created and green:
  - Happy path: correct username + correct password → 200, session set.
...
- [ ] The existing username/password form's submit target changes from `/api/auth/test-login` to `/api/auth/login`.
- [ ] The password input is `type="text"` (visible, so students can verify the passphrase as they type).
- [ ] Labels read "Username" and "Passphrase".
- [ ] On `200`: `window.location.assign('/account')`.
- [ ] On `401`: inline error message shown below the form.
