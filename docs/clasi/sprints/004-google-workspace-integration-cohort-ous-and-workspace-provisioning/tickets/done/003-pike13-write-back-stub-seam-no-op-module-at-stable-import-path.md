...
- [x] File `server/src/services/pike13-writeback.stub.ts` is created.
- [x] The module exports `leagueEmail(userId: number, email: string): Promise<void>`.
- [x] The implementation logs at INFO level:
      `"pike13-writeback: leagueEmail deferred to Sprint 006 — no-op call site"`
      with `{ userId, email }` in the log context.
- [x] The module exports `githubHandle(userId: number, handle: string): Promise<void>`
      (a stub for the other write-back function from UC-020, consistent with
      the spec — Sprint 006 will also need this).
- [x] `githubHandle` logs equivalently: `"pike13-writeback: githubHandle deferred to Sprint 006"`.
- [x] Neither function throws under any circumstance.
- [x] The module is importable; a simple unit test (or smoke import) confirms
