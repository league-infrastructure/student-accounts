...
8. App runs the merge similarity check against all existing Users (see UC-013).
...
8. App runs merge similarity check (see UC-013).
...
- Staff sees the org-wide student list, read-only (per §8.3).
...
5. App runs merge similarity check for each newly created User (see UC-013).
...
- Target User has an active League Workspace ExternalAccount (type=workspace).
  Provisioning a Claude seat without a League Workspace account is not
  permitted — the seat invitation is sent to the student's League address.
...
3. App uses the User's League Workspace address as the invite address.
...
- Seat invitation delivered to the User's League Workspace inbox.
...
5. If provider=github: app writes the GitHub username to the User's Pike13
   record (custom field "GitHub Username"), if Pike13 is linked (see UC-016).
...
6. If confidence < 0.6: the pair is discarded; no record created.
...
2. App displays a list of all Users with role=student across the organization
   (org-wide, per §8.3 — no per-cohort scoping for staff).
