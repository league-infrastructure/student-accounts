...
status: active
source: docs/user-account-management-requirements.md
---
...
provisioned a Claude seat or student email. The app is purely a read/administer
tool for them.
...
- Provision a Claude Team seat. The app creates the seat in the configured
  Claude Team workspace. The seat is tied to one of the User's Logins
  (typically the League Google account once it exists, or the primary email
  otherwise — confirmed by §8 resolution 1).
...
3. High-confidence suggestions appear in an administrator merge queue. The
   administrator reviews each pair side by side and approves, rejects, or
   defers.
4. On approval, the two User records are merged: Logins, External Accounts,
...
Staff see the full user directory org-wide, read-only. Read-only on profile
and External Account status. No provisioning, suspension, removal, merge, or
...
- **Invite or add a seat.** The seat invitation is sent only to the
  student's League Workspace address. A student cannot be provisioned a
  Claude Team seat until they have a League Workspace account — granting
  Claude access is in fact one of the primary reasons the student Workspace
  account exists. The primary (external) email is not involved in the seat
  invitation.
- **Suspend a seat.**
...
Confidence thresholds (tunable defaults per §8.7):

- Pairs scoring **≥ 0.6** surface in the merge queue for administrator review.
- Pairs scoring **< 0.6** are discarded without being shown.

A single threshold is used (not a three-tier surface/review/discard split) to
keep the queue from becoming a dumping ground of weak matches.
...
1. **Claude seat email binding** — invite is sent only to the student's
   League Workspace address. The student's external primary email is not
   part of the Claude seat invitation. A Claude Team seat cannot be
   provisioned until the League Workspace account exists.
2. **Merge scanner cadence** — similarity check runs on every new User
...
3. **Staff scoping** — staff identity comes from membership in the League
   ...
   dimension — they see students across the organization, read-only.
...
6. **Self-service provisioning** — students can request (a) a League email
   alone, or (b) a League email and a Claude seat together. Claude seat alone
   is not a valid request because the seat depends on the League address.
7. **Haiku confidence thresholds** — proposed defaults, tunable: pairs scoring
   ≥ 0.6 surface in the merge queue for administrator review; pairs below 0.6
   are discarded without being shown. A single threshold, not a three-tier
   surface/review/discard split, keeps the queue from becoming a dumping
   ground of weak matches.

---

## 9. Remaining Open Questions

None at this time. All questions raised during initial requirements review
have been resolved (see §8).
