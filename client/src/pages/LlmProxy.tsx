/**
 * LlmProxy page — mounted at /llm-proxy under AppLayout.
 *
 * Extracted from LlmProxySection in Services.tsx (Sprint 021, ticket 002).
 * Fetches LLM proxy status from /api/account/llm-proxy and renders the full
 * proxy card: endpoint, token, quota bar, and usage snippet. Handles loading,
 * error, and "not enabled" states the same as the original LlmProxySection.
 */
...
export default function LlmProxy() {
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.pageTitle}>LLM Proxy</h1>
      {renderCard()}
    </div>
  );

  function renderCard() {
    // Hide gracefully on fetch error — same behaviour as LlmProxySection.
    if (err) return null;
