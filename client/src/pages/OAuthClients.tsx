/**
 * OAuthClients page — /oauth-clients (Sprint 020).
 *
 * Moved out of AdminLayout (was /admin/oauth-clients in Sprint 018).
 * Now mounted under AppLayout for all authenticated users.
 *
 ...
 *
 * API base: /api/oauth-clients (Sprint 020 — was /api/admin/oauth-clients)
 */
...
// ---------------------------------------------------------------------------
// Scope checkbox group component
// ---------------------------------------------------------------------------
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 4 }}>
      {SUPPORTED_SCOPES.map((scope) => (
        <label key={scope} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.6 : 1 }}>
          ...
          />
          <span style={{ fontFamily: 'monospace', fontSize: 13 }}>{scope}</span>
        </label>
      ))}
    </div>
  );
...
export default function OAuthClients() {
  ...
  const createMutation = useMutation<{ client: OAuthClientRow; client_secret: string }, Error, CreateClientBody>({
    mutationFn: (body) =>
      apiFetch('/api/oauth-clients', { method: 'POST', body: JSON.stringify(body) }),
    onSuccess: (result) => {
  ...
  const rotateMutation = useMutation<{ client_secret: string }, Error, number>({
    mutationFn: (id) =>
      apiFetch(`/api/oauth-clients/${id}/rotate-secret`, { method: 'POST' }),
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: ['oauth-clients'] });
  ...
  const disableMutation = useMutation<void, Error, number>({
    mutationFn: (id) =>
      apiFetch(`/api/oauth-clients/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['oauth-clients'] });
